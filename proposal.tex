...
\begin{figure*}[ht]
    ...
    The binary classifier predicts if each the restaurant properties from each row should can be presented in the answer (see the grey column). 
    Note that the input words and the database entities are represented as embeddings which are trained. 
    The row embeddings are combined embeddings of embeddings of properties values where each property is stored in one column.
