...
\label{fig:gan}
\end{figure}
\todo{Why the footnote (?6?) is not visible?}
...
The system should access the database and create and entry to database.
...
Our first experiments showed that we have too few data and our model is too complicated to learn anything meaningful. 
We used the DSTC2 dataset and we tried to mimic the system behaviour.
We heuristic function to simulate DB calls as collected in work of~\cite{platek2016wochat} as input features and which showed promising results in~\cite{platek_recurrent_2016}. 
...
It effectively produces sophisticated lies mimicking the systems replies and randomly choosing facts from database.
Consider example from Figure~\ref{fig:encdec_lies} where both plain encoder-decoder and our propose models made same type of mistakes.
...
\begin{figure}[!ht]
    ...
    \caption{For encoder-decoder framework is easy to learn templates but hard to extract semantic information from the query and the system database. Note, that the decoded reply produced telephone number but from completely different restaurant (not even the one called {\bf meghna}).}
...
\begin{figure*}[!bt]
    \centering
    \includegraphics[width=1.0\linewidth]{todo}
    \caption{Computational graph of neural network model which demonstrates conditioning on the database part was completely ignored by the trained model.}
\label{fig:e2end_fail}
\end{figure*}

We aim to solve the problem by simplifying the model and learn to predict each row of database directly, basically as set of binary classifiers. 
The advantage is that such architecture allows for multiple possible results which are typical for user queries.
As consequence each dialogue history and corresponding system reply training example will be expanded using heuristic functions to many possible replies given the history and the system will be taught by more examples.
We expect that by explicitly preparing the model for the data ambiguity the model will generalize better and learn to propagate constraints from history over database to results which will be semantically more accurate.
We are able to measure automatically the semantic compatibility based on the dialogue state annotation from DSTC2 data or DB calls in the dataset~\cite{platek2016wochat}.
\begin{figure*}[ht]
    \centering
    \includegraphics[width=1.0\linewidth]{todo}
