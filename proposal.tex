...
Such representation can be evaluated using easy to understand and widely accepted measures such accuracy and L2 measure.\footnote{DSTC2 and DSTC3 challenges recommend using accuracy and L2 measures. In addition, one is advised not to evaluate the dialogue state trackers on the first turns where the dialogue state does not change.}
...
As a result of the DST challenge success, the dialog state tracking of state described by manually designed ontology was widely adopted as used it was used in the DST challenges.
...
\end{itemize}
Neural networks models not only achieve the best results in all the tasks but the models are relatively straightforward to combine and optimize jointly.
The problem is to provide enough labeled data for the neural networks since they need to learn a lot of unspecified structure and patterns
Luckily, by using neural network models which are modelling the structure of task latently one is able to collect more data in cheaper way because less annotation is needed.
.
