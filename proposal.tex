...
\begin{itemize}
    \item The prediction of next decision using the sequence-to-sequence model~\cite{bahdanou,todo} are conditionally independent given the sufficient statistics of the hidden state of the decoding RNN.
...
Furthermore, multi weak reward functions can be combined to form much stronger feedback signal.~\cite{todo}
...
One of key properties of dialogues systems is that their domain changes over time~\cite{todo}, so every predefined loss function becomes obsolete after some time.
...
The IRL is ill-posed problem in general~\cite{todo} and is guaranteed to work only for special cases e.g.~\cite{abbeel_apprenticeship_2004}.
In our experiments (see~Section~\ref{sec:feedback}) we aim to learn feedback from interactive conversation, but we plan to use IRL and similar approaches as reward shaping~\cite{su-todo} only for comparison.
...
\todo{REWRITE THE MOST IMPORTant SECTION after experiments}
A feedback in statistical models have different form.
We introduced supervised learning and cross entropy for neural networks in~Section~\ref{sec:loss}.
Supervised learning expects prepared annotation - labeled training data.
Reinforcement learning is more general framework for updating parameters of statistical models and also may be used for neural networks~\cite{williams2016end,policy_rec_gradients}.
In dialogue system it is typically used online when a deployed system collect simple feedback at the end of each dialogue, or more recently the RL is used to fine tuned supervisely trained neural network by optimizing directly the evaluation function~\cite{su}
