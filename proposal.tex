...
The most effective approaches either use a~very weak feedback and improve deployed dialogue system via reinforcement learning or use supervised learning and labeled data.
This work build on the success of then methods above and focuses on designing the conversational agents which will be able to:
\begin{itemize}
...
Several research groups deployed such speech-to-speech dialogue system on different domains, for example:
\begin{itemize}
    \item Let's go system~\cite{raux_lets_2005} helped the participants of experiments book a flight ticket.
    \item The Cambridge group repeatedly uses Cambridge restaurant domain to evaluate experiments on crowdsourced users where users search for a restaurant in Cambridge.
...
The dialogue state is commonly defined by manually designed domain ontology.
Thanks to the DST challenge success, where a handcrafted ontology was provided, it may seem easy to design such ontology\footnote{The Cambridge restaurant ontology had been polished over several years of research and it is actually rather simple.}.
In general, we see the manually designed ontology as arbitrary, costly and also error prone process.
...
It reduces the task of building text-to-text dialogue system to training a single statistical model and thus optimizes the response generation process jointly and avoids cumulating errors.
...
As consequence, the data for intermediate representation between the components does not to be provided for training the pipeline, but typically larger amount of the training examples is needed.
...
The model is trained not only from dialogue conversation transcriptions and the domain DB but also it uses dialogue state annotations for decomposing the system into multiple components.
...
Note that the cross-entropy training is the most successful method how to train neural networks.
However, maximizing likelihood of next word in response should not be the ultimate goal which conversational agents should achieve.
...
The work of~\cite{nouri_cultural_2012} uses successfully IRL for a toy task in cultural decision making in negotiations.
...
The work of~\cite{pappu_knowledge_2014} detects errors over multiple components and then employs post processing step which optimize the pipeline of components jointly.
This attitude is no longer necessary because all our components are optimized jointly.
However, the work shows interesting insights what might be the most common errors and it also uses an~error discovery for knowledge acquisition.
...
The most importantly, we want collect annotations for improving the system itself in addition to learning new facts to directly.
...
Our goal is to develop strategies which will help a conversational agent collect annotations and facts useful to any statistical inference algorithm.
